...
const std::array<const Enemy::Em, Enemy::EM_MAX> Enemy::str =
{{
    {"(None)",                  {0,  NORMAL}},
    {"Rathian",                 {1,  NORMAL}},
    {"Gold Rathian",            {1,  RARE}},
...
const char* Enemy::GetStr(const Enemy::ID& _id)
{
    for (const auto& item : Enemy::str)
        if (item.id.id == _id.id && item.id.idSub == _id.idSub)
            return item.name;
